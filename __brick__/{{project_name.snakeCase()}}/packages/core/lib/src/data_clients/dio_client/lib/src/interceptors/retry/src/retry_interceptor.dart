...
import 'package:core/src/data_clients/dio_client/lib/src/interceptors/retry/retry.dart';
...
class RetryInterceptor extends Interceptor {
  ...
  Future<bool> _shouldRetry(final DioException error, final int attempt) async {
    try {
  ...
  Future<dynamic> onError(
    ...
    try {
      await dio.fetch<void>(requestOptions).then(handler.resolve);
    } on DioException catch (e) {
      super.onError(e, handler);
    }
  }

  Duration _getDelay(final int attempt) {
  ...
  RequestOptions _recreateOptions(final RequestOptions options) {
  ...
  void _printErrorIfRequestHasMultipartFile(final RequestOptions options) {
    if (_multipartFileChecked) return;
    if (options.data is FormData) {
      final data = options.data as FormData;
      if (data.files
          .any((final pair) => pair.value is! MultipartFileRecreatable)) {
...
extension RequestOptionsX on RequestOptions {
  ...
  set _attempt(final int value) => extra[_kAttemptKey] = value;

  bool get disableRetry => (extra[_kDisableRetryKey] as bool?) ?? false;

  set disableRetry(final bool value) => extra[_kDisableRetryKey] = value;
}

extension OptionsX on Options {
  bool get disableRetry => (extra?[_kDisableRetryKey] as bool?) ?? false;

  set disableRetry(final bool value) =>
